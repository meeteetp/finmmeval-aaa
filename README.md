...
| **Tuning research** | `backtest.ipynb` — fetches SEC EDGAR filings, computes per-row features, runs 4-fold expanding-window cross-validated grid search. End-to-end reproducible. |
| **Selected configuration** | `tuned_params.json` — winning weights, thresholds, per-fold test metrics, and aggregate stats. |
| **Filing data** | `filings_tsla.json` — Risk Factors / MD&A / Forward-Looking sections of TSLA 10-K and 10-Q filings, fetched from SEC EDGAR. Read by both the backtest and the live endpoint. |
| **Performance figures** | `figures/*.png` — out-of-sample cumulative return, drawdown, position timeline, rolling Sharpe, and per-fold CR plots. Generated by the backtest notebook. |
...
Reported as the mean across 4 expanding-window CV test folds. The numbers below are from the most recent CV run (after EDGAR Risk Factors / MD&A / Forward-Looking sections were added to the TSLA feature set). Re-running `backtest.ipynb` regenerates this table along with all figures below.

| Asset | Strategy CR | Buy-and-hold CR | Sharpe | Worst MaxDD |
|---|---:|---:|---:|---:|
| BTC  | **+7.4%** | −4.8% | **+1.62** | −7.3% |
| TSLA | **+6.1%** | −5.4% | **+2.22** | −5.0% |

Both assets beat buy-and-hold on the primary CLEF metric (Cumulative Return) — TSLA on every fold, BTC on 3 of 4 folds. The EDGAR Risk Factors / MD&A / Forward-Looking sections nearly doubled TSLA's CR (vs the news-summary-only variant) and pushed Sharpe past 2.

### Cumulative return (out-of-sample, stitched across CV folds)

The strategy curves below are computed *only* on the held-out CV test slices stitched chronologically. Dotted vertical lines mark fold boundaries.

![BTC OOS cumulative](figures/oos_btc.png)
![TSLA OOS cumulative](figures/oos_tsla.png)

### Drawdown comparison (underwater plots)

Strategy drawdowns versus buy-and-hold over the OOS window. The defensive bias keeps strategy drawdowns shallow even when the asset itself drops sharply.

![BTC drawdown](figures/drawdown_btc.png)
![TSLA drawdown](figures/drawdown_tsla.png)

### Strategy positions over time

Top panel: asset price across the OOS window. Bottom panel: daily position (LONG / FLAT / SHORT). Reveals when the model leaned into trends vs stepped aside.

![BTC positions](figures/positions_btc.png)
![TSLA positions](figures/positions_tsla.png)

### Rolling 30-day Sharpe ratio

Annualized Sharpe over a trailing 30-day window. Stable positive bands indicate the risk-adjusted edge is consistent rather than dependent on a few favorable stretches.

![BTC rolling Sharpe](figures/rolling_sharpe_btc.png)
![TSLA rolling Sharpe](figures/rolling_sharpe_tsla.png)
...
Daily request (00:00 UTC)
   ...
   FastAPI /                   ┌────────────────────────────────────────┐
        ...
        │                      │   • Filing source: organizer summary   │
        │                      │     + EDGAR Risk Factors / MD&A /      │
        │                      │       Forward-Looking (forward-filled  │
        │                      │       from filings_tsla.json)          │
...
```
**TSLA — tighter symmetric thresholds, EDGAR filings + peer returns dominate:**
```
weights : w_news=0.6  w_filing=0.6  w_disagree=0  w_trend=0  w_mom=0.5  w_peer=1.0  w_sector=0  w_macro=0
buy_th  : +0.12   sell_th: −0.12
```
CV emphasized EDGAR-derived filing tone (Risk Factors / MD&A / Forward-Looking, forward-filled), peer 5-day returns (RIVN/GM/F/NIO/LCID), and news sentiment, with thresholds tightened to ±0.12 to capture more of the available signal. Trend, sector ETF, macro, and disagreement were zeroed.
...
For BTC, this design produced an outcome that is methodologically meaningful: the super-defensive asymmetric pair (BUY +0.30 / SELL 0.00) won — over the purely symmetric candidates. A defensive risk preference was therefore *not* hand-imposed onto the model; it was offered as one of several candidates and *validated by data*. For TSLA, the symmetric ±0.12 pair won — meaning the data did *not* support a defensive asymmetric prior on TSLA. CV instead tightened the thresholds (from ±0.20 in the pre-EDGAR variant to ±0.12 here) once the richer filing signal made smaller score magnitudes meaningful.
...
| Path | Purpose |
|---|---|
| `app.py` | FastAPI server; loads FinBERT, MiniLM, and `filings_tsla.json` at startup; per-request hybrid scoring with the CV-selected weights and thresholds baked in. |
| `requirements.txt` | Python dependencies (FastAPI, transformers, sentence-transformers, yfinance, etc.). |
| `Dockerfile` | Containerizes `app.py`; pre-downloads FinBERT and MiniLM into the image; copies `filings_tsla.json` into the image so cold starts are instant. |
...
| `backtest.ipynb` | Full reproducible research notebook — fetches SEC EDGAR filings, loads the official dataset, computes per-row features (no lookahead), runs 4-fold expanding-window CV grid search, plots and saves out-of-sample performance figures, writes `tuned_params.json`. |
| `filings_tsla.json` | Risk Factors / MD&A / Forward-Looking sections of TSLA 10-K and 10-Q filings, fetched from SEC EDGAR via `sec-edgar-downloader` and parsed by the notebook. Used by both the backtest and the live endpoint via forward-fill (each date sees the latest filing available as of that date). |
...
| `figures/` | OOS performance charts produced by `backtest.ipynb`: cumulative returns, drawdowns, position timelines, rolling Sharpe, per-fold CR. Embedded in this README. |
...
2. Fetches TSLA 10-K and 10-Q filings from SEC EDGAR (via `sec-edgar-downloader` + SEC submissions API), extracts Risk Factors / MD&A / Forward-Looking sections, and saves `filings_tsla.json`.
3. Computes 9 features per row with no lookahead. For TSLA, filing-derived signals are forward-filled by date so each row sees the latest filing available as of that date.
4. Splits each asset into 4 expanding-window CV folds (train always older than test).
5. Grid-searches weights and threshold pairs. Threshold candidates encode a range of human risk-preference priors (see "Threshold candidates as human priors" above).
6. Selects the configuration with the best **average test-slice objective across all 4 folds**.
7. Reports per-fold and aggregate test metrics; writes `tuned_params.json`.
8. Saves all OOS performance charts into `figures/`.
...
- Python: 3.11 (3.9 also works for the notebook; the live endpoint runs Python 3.11 inside Docker)
...
- Filing data: SEC EDGAR via `sec-edgar-downloader`, with filing-date metadata from `https://data.sec.gov/submissions/CIK0001318605.json` (SEC's official public data API; no auth required, only a `User-Agent` header per SEC policy)
- External market data: `yfinance` (no key) — peers (RIVN, GM, F, NIO, LCID for TSLA; ETH-USD, COIN for BTC), sector ETF (XLY for TSLA, COIN for BTC), VIX
